        ...
        try{
            if(lastCrime !== null && cooldown - (Date.now() - lastCrime) > 0){
                ...
                if(result < 5){
                    ...
                    const embed = new Discord.MessageEmbed()
                    .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL())
                    .setDescription(`<:emojiyes:779190801392861224> ${crimeSuccessReplies[Math.floor(Math.random() * crimeSuccessReplies.length)]}! You got ${successamount} coins!`)
                    .setColor('GREEN')
                    .setFooter('The EverythinBot')
                    .setTimestamp(new Date())
                    return message.channel.send(embed)
                 ...
                 if(result > 5){
                    ...
                    const embed = new Discord.MessageEmbed()
                    .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL())
                    .setDescription(`<:emojino:779190801598775317> ${crimeFailReplies[Math.floor(Math.random() * crimeFailReplies.length)]}! You lost ${failamount} coins!`)
                    .setColor('RED')
                    .setFooter('The EverythinBot')
                    .setTimestamp(new Date())
                    return message.channel.send(embed)
